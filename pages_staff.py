...
<style>
  ...
  .avail-panel { border: 1px solid var(--line); border-radius: 14px; background: var(--surface); padding: 22px 24px; margin-bottom: 20px; }
  ...
  <div id="dashScreen">
    ...
    <div class="avail-panel" id="availPanel">
      <div class="avail-head">
        <h3>Cabana Availability</h3>
        <input type="date" id="availDate">
      </div>
      <p class="field-hint" style="margin:0 0 14px;">A live view of who's holding which cabana on a given date &mdash; separate from the table below, which lists every request regardless of date.</p>
      <div id="availStatus" class="cabana-map-status">Loading&hellip;</div>
      <div id="availMap"></div>
    </div>
    ...
    <div class="table-scroll">
      <table>
        ...
        <tbody id="bookingsBody">
          <tr class="empty-row"><td colspan="13">Loading bookings&hellip;</td></tr>
        </tbody>
      </table>
    </div>
  </div>
...
<div class="modal-backdrop" id="addModalBackdrop">
  <div class="modal">
    ...
    <form id="addForm">
      ...
      <div class="modal-field">
        <label for="addCabana">Cabana (optional)</label>
        <select id="addCabana"><option value="">Not assigned yet</option></select>
      </div>
      <div class="modal-row">
        <div class="modal-field" style="margin-bottom:0;">
